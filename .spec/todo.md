...
- [x] U72 [UX] collection 頁（截圖為 `/c/crops`）手機寬度下，搜尋框旁的
  「篩選 ▲」鈕明顯比搜尋框高一截，兩者底部沒對齊，視覺上像跑版、質感
  變差。

  **成因**：`CollectionPage.jsx:82` 的篩選鈕有 `min-h-11`（44px，
  `md:min-h-0` 只在桌機拿掉）——刻意的觸控目標大小規則（DESIGN.md
  §響應式行為「觸控目標 ≥ 44px」），但同一列的搜尋框 `<label>` 沒有
  對應的 `min-h-11`，手機寬度下兩者高度對不齊。

  **執行（2026-07-24，使用者用 AskUserQuestion 選定「篩選鈕改用透明熱區
  撐大」方向）**：篩選鈕拿掉 `min-h-11 md:min-h-0`，改回跟搜尋框一樣的
  視覺尺寸（`px-3 py-1.5`，無 min-height）；鈕本身加 `relative`，新增
  `before` 偽元素（`absolute inset-x-0 top-1/2 h-11 -translate-y-1/2`）
  蓋出 44px 高的透明熱區疊在可視框之上，點擊偽元素仍會冒泡到
  `<button>` 本身、不影響版面；`md:before:hidden` 桌機不需要這層熱區
  （桌機原本就沒有 44px 限制）。搜尋框樣式完全沒動。
  驗證：`npm run lint`／`npm test`（232）／`npm run build`（警告維持 55）
  皆綠；Playwright 讀兩者 `boundingBox()` 確認可視高度都是 32px、底部
  對齊誤差 0px；並在按鈕可視框上緣以外 15px 處（在 44px 熱區內、可視框
  外）模擬點擊，確認篩選面板仍會展開，證實熱區撐大生效、不是只有視覺
  對齊。375px 截圖目視確認搜尋框與篩選鈕已無高度落差。
