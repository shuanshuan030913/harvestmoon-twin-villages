...
- [ ] U60 [UX] 料理總覽模式（U56）卡片視覺混亂，待使用者拍板修法方向：
  截圖顯示「烤魚」（食材欄「任一種魚都可以」，12 個「或」選項）卡片被撐得
  極長，同排相鄰卡片（如只有 2 樣食材的「蟹肉滑蛋」）相對之下留下大片空白，
  整排看起來很不平衡；`ItemChips` 目前把整個「或」群組攤平成一整排 chips，
  沒有收合機制。核對 `recipes.json` 發現不是單一個案——`ingredientsLinks`
  裡「或」選項數 ≥5 的至少有：紅燒魚/烤魚（12）、生魚片（8）、豆漿鍋/嫩煎魚排
  （6）、燉菜/散壽司/味噌湯（5），這類「彈性食材」的菜色都會有同樣的卡片
  過長問題，不是只有烤魚一筆。**本項僅記錄問題與初步成因分析，不預設修法**
  ——使用者中斷過一次直接動手的嘗試並糾正「應該先確認要怎麼改」，這裡刻意
  只列可能方向供討論，不逕自選一個實作：
  - 方向 A：`ItemChips.jsx` 對過長的「或」群組加收合／展開（門檻可討論，
    例如超過 4 個顯示前幾個＋「還有 N 種▼」，手法可沿用同檔案既有的
    `CategoryChip` 展開/收合互動，不用新造視覺語言）。
  - 方向 B：只在總覽模式卡片限定收合／截斷，其他用到 `ItemChips` 的地方
    （角色禮物清單等）維持現狀，不放大改動範圍。
  - 方向 C：從卡片版面本身下手，例如總覽模式的卡片不用 `h-full` 撐滿同排
    高度，讓短卡片維持自己的高度，不被拉伸出空白（但可能讓同排卡片高低
    參差，是否比現狀更好看需要實際比對）。
  - 也可能是上述幾種的組合，或使用者有其他想法。
