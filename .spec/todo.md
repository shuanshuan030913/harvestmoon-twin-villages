...
- [ ] U58 [Interface] `.spec/modules/query-system.md` 補一條正式審查標準：
  ...
  抓到，不是系統性檢查出來的。要補的標準（草案，實際文字認領時再定稿）：
  新增或調整 collection 的 `columns`／`detailColumns` 設計時，須檢查明細頁
  「獨有」內容（不在列表欄位、也不跟列表欄位重複的部分）是否單薄且不具
  跨條目鑑別度；若是，優先考慮把該內容併入列表本身（比照 U56 總覽模式做法：
  把有意義的獨有資料結構化後直接攤在列表卡），而不是預設維持「列表精簡＋
  明細補完整」兩層分拆。**本項僅記錄使用者已同意的方向，尚未落筆**——認領
  時把上述草案文字實際寫進 `query-system.md`，並藉這次機會過一遍全站 9 個
  collection 現況（本輪口頭核對：characters/crops 明細獨有內容量大，判定
  不受影響；fishes/insects/items 列表欄位本身已經夠豐富，明細稀薄但沒有
  「藏資料」的問題；festivals/villages 未細查；已確認需要處理的只有
  minerals，見 U59）。
