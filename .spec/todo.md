...
- [ ] U62 [UX] `GameDialog.jsx`（全站對話框關閉鈕）與 `AnimalTracker.jsx`
  ...
  **範圍需要跟使用者確認**：`GameDialog.jsx` 的關閉鈕（每個對話框右上角，
  ...
  框未明確表態。**使用者已確認範圍，並二度細化 `DeleteAnimalDialog`
  的處理方式**：①`GameDialog.jsx` 只拿掉圓形描邊改裸 ×；②`DeleteAnimalDialog`
  觸發鈕**不是維持現狀，是換成垃圾桶 icon**（不再是 × 字，語意從「關閉/
  取消」換成更直接的「刪除」動作圖示）。
  **本項記錄方向已拍板，尚未實作**——認領時要處理：
  ...
  - `DeleteAnimalDialog`：`icons.jsx` 現有 sprite（person/wheat/sheep/pot/
    ...
    `aria-label` 目前是「刪除 {nickname}」，語意不變不用改，只換視覺
    （文字 × 換成 `<Icon id="trash">`），觸控熱區沿用現有 `h-8 w-8`
    圓形容器即可（這裡沒有像關閉鈕那樣「拿掉框」的訴求，維持圓形按鈕
    容器，只是裡面的內容從文字 × 換成圖示）。
