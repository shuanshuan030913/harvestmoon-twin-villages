...
- [x] U58 [Interface] `.spec/modules/query-system.md` 補一條正式審查標準：
  ...
  不受影響（略過重跑）。
- [ ] U59 [UX] 礦物列表／明細收整（U20 驗證過的觀察，這次正式處理，
  dep: 建議跟 U58 標準文字定稿後一起認領，但技術上可獨立動工）：礦物明細頁
  （核對奧利哈鋼條目）內文只有「礦山隧道內的礦物。」一句，跟列表已顯示的
  `location`/`sell_price`/`use` 完全重複；`location` 欄本身 19 筆同值零
  鑑別度（U29，2026-07-20 已移出列表但仍留在明細頁，即為 U20 觀察到的
  「明細比列表多的東西沒有鑑別度」）。**明細頁唯一真正獨有的內容**是一張
  星度賣價表（☆0.5–☆5.0 六階賣價），但目前只存在 `entry.html`／`entry.plain`
  純文字裡（`build-content.js` 未解析成結構化欄位），沒有辦法直接顯示在
  列表卡上。做法方向：①比照 U56 手法，把星度賣價表從 markdown 表格解析成
  結構化欄位（如 `sell_price_by_star: { '0.5': 3200, '1.0': 3840, ... }`），
  ②列表卡或總覽模式攤開顯示六階賣價，③明細頁的 `location` 欄與重複敘述句
  可以拿掉（19 筆同值的「礦山隧道（雙村共通）」改成 `/c/minerals` 列表頁
  頂部一次性的頁面說明，不用每條目重複），④重新評估 minerals 是否還需要
  維持獨立明細頁（拿掉重複內容＋搬走星度表後，明細頁可能所剩無幾）。
  **本項僅記錄使用者已同意的方向，尚未實作**——認領前需要規劃：星度賣價表
  6 階是否 19 篇全數齊備（若有缺值需比照既有「缺值不渲染」原則處理）、
  結構化欄位的解析寫在 `build-content.js` 哪個階段、列表卡加 6 個數字後
  手機版排版是否需要另外設計（不能直接沿用 U56 的 chips 換行寫法，星度
  賣價是有序數列不是無序集合，呈現邏輯不同）。
