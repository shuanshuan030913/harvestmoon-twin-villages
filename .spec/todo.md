...
- [!] T6.12 [UX] 類別食材與遞迴料理連結：ingredients 中類別食材（きのこ類）點擊展開可用清單、料理作食材遞迴跳 recipes（驗證：recipes 條目化後——主食類含きのこ類項可展開清單；烏龍麵→狐狸烏龍麵可遞迴跳轉）(dep: T6.6) blocked：recipes.json 現為空陣列（C2 內容缺口），無資料可驗證 ingredients 展開/遞迴行為，待 C2 補齊後解除
