...
- [ ] U60 [UX] 料理總覽模式（U56）卡片視覺混亂，待使用者拍板修法方向
  ...
  - 框語言：廚具標籤用 `border-ink/40 rounded px-1.5 py-0.5`（純矩形描邊、
    ...
    新增時漏對齊。
  **本項僅記錄問題與成因分析，不預設修法**——使用者已經中斷過一次直接動手
  的嘗試並糾正「應該先確認要怎麼改」，這裡刻意只列可能方向供討論：
  - 方向 A：廚具標籤拿掉獨立的矩形描邊框，改成純文字（比照 U54 拿掉點心
    列描邊「不畫框也不塗底色，純靠間距排節奏」的做法），字級升到 `text-sm`
    符合既有規則，不再是第三種框語言。
  - 方向 B：廚具乾脆比照食材，也做成 `chip-torn` 樣式的 chip（跟食材 chips
    同一種語言，只是內容不同），卡片裡只剩一種「框」規則。
  - 方向 C：其他調整（如統一卡片內所有文字字級成同一級、只用顏色深淺分層
    不用字級分層），或使用者有其他想法。
  **認領前必須先跟使用者確認要選哪個方向（或都不是），拍板後才動手**，
  不可逕自判斷「看起來很直覺」就跳過確認。
