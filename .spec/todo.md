...
- [ ] U67 [UX] `EntryCard.jsx` 的 `RecipeCard`（U66 剛拿掉切換鈕、變成 recipes
  ...
  張料理卡都有同樣問題，不是單一個例。
  **本項僅記錄回饋，尚未落地實作**——修法方向是把 `<a>` 從只包名稱/價格
  那行，改成整個包住卡片內容（名稱/價格＋廚具＋食材都在 `<a>` 內），
  `<div>` 容器可能因此不再需要（`<a>` 直接扛起 `bg-(--village)/10` 等版型
  class）。**要注意**：U60 已裁決食材文字不再是可點連結（`ItemChips`
  的可點特性拿掉，改純文字），這裡的訴求是「整張卡跳去該料理明細頁」，
  不是「食材各自連到食材條目頁」——兩者不衝突（食材文字仍是純文字，只是
  現在被包在通往料理明細頁的大 `<a>` 裡面），但認領時要確認理解一致，
  不要誤把 U60 的決定連帶改回去。
