...
- [x] U64 [UX] `CollectionPage.jsx` 的搜尋框＋「篩選」切換鈕列應該 `sticky`
  ...
  `sticky top-0 z-10`，這裡只要疊一層 `sticky`（`top` 要扣掉全域 header 的
  實際高度，`z-index` 也要比 `z-10` 低，避免蓋過全域 header 或被蓋過）即可，
  不是新技法。**使用者已確認①**：篩選面板（`FilterBar`）展開時也要一起
  sticky，不是只黏搜尋框那一列。**本項記錄範圍已確認一半，尚未實作**——
  認領前仍需要處理：②sticky 區塊需要不透明背景（`bg-cream`，卡片底色），
  捲動時才不會被下方內容穿透看到；③篩選面板展開時連同搜尋框一起黏住會佔
  掉不少垂直高度（尤其 recipes 篩選群組多、行動版視窗本來就矮），需要實際
  測過收在合理範圍，必要時面板展開狀態下是否要另外處理（例如限制面板本身
  ...
  **2026-07-23 追加修正（使用者截圖回饋「線多到很醜」）**：sticky 區塊自己
  收尾的 `border-b-2 border-dashed` 跟正上方全域 header 自己的虛線貼太近，
  變成兩條虛線疊在一起，讀起來雜亂；出 artifact（現況 vs A 陰影 vs B 細
  實線兩個方向）對稿，使用者選 A。改法：拿掉 `border-b-2 border-dashed`，
  改用 `shadow-[0_4px_6px_-4px_rgba(74,55,40,0.18)]` 表達「這塊浮在內容
  上面」，不再畫邊框線。驗證：`npm run lint`／`npm test`（229）／
  `npm run build` 皆綠；本機起 Playwright 在 390／1280px 截圖核對捲動後
  雙虛線問題消失、改為柔和陰影分隔，console 無錯誤。
...
- [x] U63 [Interface] guide 頁同時出現內文「## 來源」清單與頁尾「原始出處」
  ...
  frontmatter 都有 `source` 欄位，50 篇 `entry.html` 也都有 `<h2>來源</h2>`
  區塊，兩者 100% 重疊。
  根源在 `scripts/build-content.js` 的 `SOURCES_SECTION_COLLECTIONS`
  （U25，2026-07-20 引入，把「## 來源」段解析成 `entry.sources`／`entry.source`
  結構化欄位、從內文剝除、統一到頁尾弱化出處列渲染，`fishes`／`crops`／
  `characters` 等條目 collection 都吃這個處理）——`guides` 從一開始就被
  排除在這份集合外，註解寫「characters/guides 已各自處理」，但這句話只
  對了一半：guides 的 frontmatter `source` 欄確實有自己的處理（`GuidePage.jsx`
  頁尾渲染「原始出處：」），**但內文自己的「## 來源」段落從沒被剝除**，
  變成兩邊都在講同一件事，只是版型/措辭不同（內文清單用「來源」標題＋逐條
  「，擷取於 YYYY-MM-DD」；頁尾用「原始出處：」＋純網址一行，且頁尾目前
  只吃單一 `entry.source`，`entry.sources` 陣列格式從沒被 `GuidePage.jsx`
  用到——`EntryPage.jsx` 的頁尾其實已經支援 `entry.sources` 陣列＋`entry.source`
  單一值兩種格式，`GuidePage.jsx` 只實作了後者）。
  **使用者已拍板方向**：「來源」跟「出處」是同一件事，應該只用一種意思、
  一套 UI 呈現，不要兩邊各講一次——即上述方向①：guides 也納入
  `SOURCES_SECTION_COLLECTIONS`，內文「## 來源」段跟其餘條目 collection
  一樣被剝除、結構化成 `entry.sources`（保留每條的擷取日期／補充說明，
  不是砍資訊，只是換位置＋統一渲染），`GuidePage.jsx` 頁尾比照
  `EntryPage.jsx` 同時支援 `sources` 陣列與 `source` 單值，全站頁尾出處列
  統一同一套渲染與措辭（用 `EntryPage.jsx` 現有的「出處：」，不是
  `GuidePage.jsx` 原本的「原始出處：」——兩者統一成同一個詞）。
  **本項記錄方向已拍板，尚未落地實作**——認領時要處理：①`extractStandardSources`
  對 guides 的「## 來源」實際格式跑一次抽取，確認 50 篇是否都符合標準
  bullet 格式（`stripSourcesSection`/`extractStandardSources` 現有的安全閥：
  格式不符會回傳 `null`，保留原段＋記警告，不靜默丟失，要看跑完後警告數
  多不多）；②`GuidePage.jsx` 頁尾邏輯改成比照 `EntryPage.jsx` 的
  `entry.sources?.length ? ... : entry.source ? ... : null` 雙格式判斷；
  ③需要跟使用者確認「guides 保留原文沿革記錄」這條原則的認定——`stripEditorialNotes`
...
- [x] U62 [UX] `GameDialog.jsx`（全站對話框關閉鈕）與 `AnimalTracker.jsx`
  ...
  裸 × 字不描邊，跟這裡的圓形框比起來更輕量。跟這個 session 已經走過好幾輪
  的「拿掉框」方向（U36/U54/U60 方向 C）同一條路線，若拿掉會更一致。
  **範圍需要跟使用者確認**：`GameDialog.jsx` 的關閉鈕（每個對話框右上角，
  影響全站，U54 才剛把它從底部實心按鈕改成這個角落圓框 × ）跟
  `AnimalTracker.jsx` 的 `DeleteAnimalDialog` 觸發鈕（畜牧追蹤每隻動物列
  的刪除入口，語意是「刪除」不是「關閉」，一般 UI 慣例對「刪除」動作不一定
  用裸 ×，可能更常見垃圾桶圖示或維持某種強調）是兩個不同語意的按鈕，使用者
  的話目前只明確指向「popup 關閉鈕」，`DeleteAnimalDialog` 是否也要一併拿掉
  框未明確表態。**使用者已確認範圍，並三度細化 `DeleteAnimalDialog`
  的處理方式**：①`GameDialog.jsx` 只拿掉圓形描邊改裸 ×；②`DeleteAnimalDialog`
  觸發鈕換成垃圾桶 icon（不再是 × 字）；③**追加修正：垃圾桶 icon 也不要
  圓形按鈕容器**——不是「維持圓形容器、只換裡面的字變圖示」，是整個
  `rounded-full border` 容器都拿掉，圖示本身就是按鈕，跟 `GameDialog`
  關閉鈕的「去框」精神一致，兩處這次都不要圓形框。
  **本項記錄方向已拍板，尚未實作**——認領時要處理：
  - `GameDialog.jsx`：拿掉框後裸 × 的可點擊範圍/觸控目標怎麼維持
    （`h-8 w-8` 這個尺寸目前同時也是熱區，改成純文字 × 若不刻意保留同尺寸
    的透明可點擊區，行動版點擊熱區會縮水，做法可以是文字 × 外面包一層
    同尺寸但無背景/無邊框的可點擊區域，視覺上看不到框但熱區不變）。
  - `DeleteAnimalDialog`：`icons.jsx` 現有 sprite（person/wheat/sheep/pot/
    fish/bug/gem/flag/village/cal/search/heart/cake/book/house/compass/
    paw/pin/box/save）沒有垃圾桶圖示，需要新增一個，比照現有規格
    （24×24 viewBox、stroke 1.8、圓端點的線條圖示，禁止 emoji）；
    `aria-label` 維持「刪除 {nickname}」不變，語意不變只換視覺；觸控熱區
...
- [x] U61 [UX] `AnimalTracker.jsx` 的 `AnimalRow` 點心累計列「難以閱讀」，
  ...
  （U54，2026-07-22 訂的列距）疊上行動版 44px 觸控大按鈕（`h-11 w-11`），
  4 列堆起來偏長，掃視要一直左右來回、上下跳動，讀起來吃力。
  出 artifact 三個排版方向並排比較（用「牛」真實數值：茶點差7／野菜差8／
  穀物差15／魚味差1）：https://claude.ai/code/artifact/c865e535-dcd8-42ae-88a6-95039798eeaa
  - 方向 A：品項/徽章/控制項收成同一叢集整列靠左，不再跨滿卡片寬度；列間
    補極細點狀線輔助掃視節奏（沿用全站既有手帳點狀虛線語言，非新框）。
  - 方向 B：4 種點心改 2×2 網格排列，垂直高度砍半、一次收進視野，代價是
    每格變窄、控制項要縮小，「還差」數字較長時可能換行。
  - 方向 C：維持直向清單，但品項名/徽章/控制項各自對齊固定欄寬（表格式），
    掃視時視覺走直線不用橫向跳動。
  **三個方向都要面對同一個取捨**：mockup 為了排版緊湊，示意圖裡的 −/＋
  按鈕畫成 26px，比 `DESIGN.md` 現行規定的行動版 44px 觸控目標小很多——
  真正落地時要嘛接受縮小違反既有規則（需要使用者裁決放行例外），要嘛要在
  版面上另外找空間維持 44px（行動版可能會比示意圖擠）。
  **使用者已選定方向（2026-07-23 追加細化，並二度修正欄數假設）**：選 C，
  但具體版型不是三方案裡畫的「單欄表格式對齊」，而是參照全站既有 `crops`／
  多欄 `EntryCard` 列表已經在用的 RWD 網格排版感覺；**使用者接著糾正**：
  不是寫死「兩欄」，欄數要看螢幕寬度自然決定，不是固定 2。也就是不套用
  `CollectionEntryList.jsx` 現有那種寫死斷點的 `grid-cols-1 md:grid-cols-2
  xl:grid-cols-3`（那是為了跟其餘 8 個分類頁共用同一套版型才寫死），而是
  單一動物卡片內部這個小網格改用 CSS Grid 的 `repeat(auto-fill,
  minmax(Npx, 1fr))`（N 為單一點心列所需最小寬度，實際數值待實作時量），
  讓欄數隨卡片實際可用寬度自然決定，不需要為特定斷點寫死欄數——因為
  `AnimalTracker.jsx` 的動物清單本身是單欄直向 `<ul>`（非 grid），每張卡片
  已經撐滿當前斷點的內容寬度，卡片內部這個小網格的欄數本來就該跟著這個
  寬度變，不是跟著全站斷點表硬對應。
  **本項另外帶出一個相關但獨立的問題（見下方新項）**：使用者質疑
  `DeleteAnimalDialog`／`GameDialog` 的「×」為什麼要用圓形描邊框起來，
  一般 popup UI 的關閉鈕通常是裸「×」不描邊——這題跟點心列 RWD 排版是
  兩件事，不要混在同一次修改裡誤觸。
  **本項記錄使用者已選定的方向，尚未落地實作**——認領前仍要先確認 44px
  觸控目標在兩欄並排時怎麼安排（兩欄版面比單欄窄，按鈕擠得下 44px 嗎），
  拍板後才動手。
  **2026-07-23 第一版做錯了，使用者截圖糾正**：第一版把
  `AnimalRow` 內部「單一動物卡片的 4 種點心」那層容器改成
  `grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))]`——但使用者的原意
  是「動物卡片本身要 RWD 塞多張」（`AnimalTracker` 的 `<ul>` 那層），不是把
  單一卡片內的 4 種點心拆欄。拆欄的後果：桌機寬度下 3 種點心一行、第 4 種
  自己換行，同一張卡片內失去原本「label 都靠左、控制項都靠右」的縱向對齊，
  使用者回饋「卡片的UI劣化了，原本還有對齊，我只要求你改成塞多個卡片，
  並不是將點心分欄」。
  **修正**：`AnimalRow` 內部點心容器改回 `flex flex-col gap-4`（跟改版前
  完全一樣，每列跨滿卡片寬度、label/badge 靠左、控制項靠右、彼此對齊）；
  RWD 自適應改套用在 `AnimalTracker` 的動物清單本身——`<ul>` 從
  `flex flex-col gap-4` 改成
  `grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-4`，讓**整張
  動物卡片**依可用寬度並排（手機窄寬度僅 1 欄＝原本一張一張直向堆疊；
  寬螢幕下 2 張以上並排），卡片內部排版不受影響。
  驗證：`npm run lint`／`npm test`（229）／`npm run build` 皆綠；本機起
  `npm run dev`＋Playwright 在 390／1000／1600px 三個寬度截圖核對：卡片內
  4 種點心對齊如初（label 靠左、控制項靠右），寬螢幕下多張動物卡片正確
  並排（1600px 寬度 2 欄＋1 欄換行），console 無錯誤。

### 2026-07-23 使用者回饋（總覽模式卡片視覺混亂，待實作）

- [x] U60 [UX] 料理總覽模式（U56）卡片視覺混亂，待使用者拍板修法方向
  （**2026-07-23 使用者第二次糾正診斷**：第一版分析誤判成因是「或」選項
  過多撐高卡片，使用者說明真正的問題是「字體或大或小 色塊線條顏色一堆」
  ——不是長度問題，是同一張卡片裡視覺語言不統一）：
  核對 `EntryCard.jsx` 的 `RecipeOverviewCard` 實際寫法，一張卡片裡疊了
  三種字級＋兩種「框」語言：
  - 字級：標題無顯式字級（繼承預設，視覺上最大）／價格 `text-sm`／廚具標籤
    與「或」文字皆 `text-xs`——三層落差集中在一張小卡片裡。
  - **廚具標籤 `text-xs` 本身牴觸 `DESIGN.md` 已經寫死的規則**（2026-07-18
    使用者裁決追加）：「其餘會被使用者當作『內容』讀的文字...一律至少
    `text-sm`（14px），不得停在 12px」，廚具是玩家要讀的內容（不是最弱連結
    如出處/清除篩選），這裡是實作時漏套既有規則，不是新爭議。
  - 框語言：廚具標籤用 `border-ink/40 rounded px-1.5 py-0.5`（純矩形描邊、
    無填色）；食材 chips 用 `chip-torn`（撕紙感不對稱圓角、cream 底色＋
    描邊）——兩種「框」外觀不同時出現在同一張卡片，加上卡片本身的村色
    半透明底色，等於一張卡片疊了 3 層不同的色塊/線條規則，這正是 `DESIGN.md`
    已經點名過的「框中框」病灶（U36/U54 處理過的同類問題），這次是總覽卡片
    新增時漏對齊。
  **使用者已拍板方向 C（2026-07-23，出 artifact 三方案 A/B/C 並排對稿後選定，
  見對話紀錄）**：全部拿掉框，廚具與食材皆改純文字＋頓號/「或」分隔的一行
  敘述，只留標題與內文兩級字級（廚具/食材字級升到 `text-sm` 符合既有規則），
  靠文字顏色深淺（ink 濃淡）分層，不靠框線分層——三方案裡改動幅度最大的
  一個：**食材個別項目會失去可點連結的特性**，跟全站既有慣例「可點物品
  集合→chips：邊框＝可以點」（`DESIGN.md`〈條目頁三語言分工〉）不一致，
  是這個方向明確要付出的取捨，使用者知情下選定。
  對稿用的 artifact 草稿：https://claude.ai/code/artifact/10b83e54-fb55-4b5f-87e1-b4134fa34feb
  （現況＋A／B／C 四版並排，用「烤魚」「蟹肉滑蛋」兩張真實卡片內容）。
  **本項記錄使用者已選定的方向，尚未落地實作**——認領時要處理：`RecipeOverviewCard`
  食材改純文字後，「食材＝可點」這個特性只在其餘用到 `ItemChips` 的地方
  （角色禮物清單、明細頁「食材」區塊等）保留，不要誤刪整個 `ItemChips`
  元件；「或」群組（最多 12 個選項）純文字化後一行可能很長，手機版排版
  是否需要換行或截斷需要另外核對。
  **2026-07-23 完成**：`RecipeOverviewCard`（`EntryCard.jsx`）拿掉 `ItemChips`
  引用；廚具改 `<p className="text-ink/55 text-sm">`（純文字，拿掉矩形描邊，
  字級從 `text-xs` 升到 `text-sm` 解除牴觸）；食材改
  `entry.ingredients.join('、')` 直接輸出純文字一行（不是重新從
  `ingredientsLinks` 組字串——`ingredients` 原始欄位每個陣列元素本來就是
  一整槽含「或」的完整字串，例如「香魚（アユ）或岩魚（イワナ）或...」，
  直接 `join('、')` 銜接槽與槽之間即可，不用另外處理「或」邏輯）。「或」群組
  換行：本來就是純文字 `<p>`，瀏覽器自然換行，不用額外處理（12 選 1 的
  「烤魚」實測換行正常，沒有溢出）。`ItemChips` 元件本身沒有刪除，角色
  禮物清單／條目頁「食材」區塊等其餘用法都不受影響。
  驗證：`npm run lint`／`npm test`（229）／`npm run build` 皆綠；本機起
  `npm run dev`＋Playwright 在 390／1280px 截圖核對「烤魚」（12 選 1 長食材
  清單）跟「蟹肉滑蛋」（2 樣食材）並排時視覺落差大幅縮小、無框線堆疊，
  console 無錯誤。

### 2026-07-23 使用者回饋（U56 只解決表面問題，追加根源修法：審查標準＋礦物，待實作）

- [x] U58 [Interface] `.spec/modules/query-system.md` 補一條正式審查標準：
  「collection 的列表＋明細兩層分拆是否划算」——使用者對 U56（料理總覽模式）
  提出質疑「你的提案是否只解決表面問題」，回頭核對後發現同一病灶在 U20
  （2026-07-21）就對 minerals 驗證過一次（明細頁 `location` 欄 19 筆全同值、
  零鑑別度，body 只有一句跟列表重複的敘述＋一張星度賣價表），但當時只是
  「驗證觀察成立」記在 U20 附帶段落裡，沒有變成正式規則，也沒有實際處理——
  導致同一種問題（明細頁獨有內容稀薄、又跟列表高度重複，逼使用者點進去卻
  拿不到新資訊）在料理這裡又被獨立發現一次，兩次都是使用者截圖抱怨才臨時
  抓到，不是系統性檢查出來的。（2026-07-24 完成）
  做法：`query-system.md` 新增規則 8「列表／明細分拆划算性審查」，定稿文字：
  新增或調整 collection 的 `columns`／`detailColumns` 設計時，須檢查明細頁
  「獨有」內容（不在列表欄位、也不跟列表欄位重複的部分）是否單薄且不具
  跨條目鑑別度；若是，優先考慮把該內容結構化後併入列表本身（比照料理總覽
  模式做法），而不是預設維持「列表精簡＋明細補完整」兩層分拆。並把
  2026-07-23 口頭核對過的全站 9 個 collection 現況一併寫入規則段落
  （characters/crops 不受影響；fishes/insects/items 明細稀薄但無藏資料問題；
  festivals/villages 未細查；animals/pets 未列入本輪；已確認需處理的只有
  minerals，見 U59）——這是文字定稿記錄，非本次重新逐一驗證。
  驗證：純文件變更，無程式碼異動，`npm run lint`／`npm test`／`npm run build`
  不受影響（略過重跑）。
- [x] U59 [UX] 礦物列表／明細收整（U20 驗證過的觀察，這次正式處理）
  （2026-07-24 完成，實際落地範圍比原草案窄——動工前先逐篇核對資料，發現
  原假設「明細頁開頭句都只是純填充句」不成立，經 AskUserQuestion 與使用者
  確認三項後才動手）：
  - **開頭句核對結果**：19 篇逐篇讀取，11 篇確實是純填充句「礦山隧道內的
    礦物／寶石。」，但另外 8 篇（謎之石版／粉紅鑽石／紅寶石／螢石／金／
    金剛石／銀／銅）帶有取得方式（雨雪限定、敲礦山石機率）、煉金材料標記、
    「最高價」等未欄位化的獨有資訊，不是重複——同 U19g 的教訓，不能假設
    「同類條目」就整批處理。
  - **使用者裁決①開頭句**：純填充句應留空、不應存在；帶獨有資訊的維持不動。
    新增 `stripMineralFillerIntro`（`entryTransforms.js`），正則只命中
    `^礦山隧道內的(礦物|寶石)。$` 這種完整無附加文字的句子，11 篇整句拿掉、
    8 篇天然不命中原樣保留。`build-content.js` 在既有 `crops`/`minerals`/
    `villages` 的 h1 剝除（U24）之後加一個 `minerals` 專屬分支呼叫。
  - **使用者裁決②列表卡**：星度賣價表**不**結構化解析、列表卡維持現狀只顯示
    `sell_price`（☆0.5），不做 6 階數字或總覽模式——原草案①②兩步驟本次
    不執行，星度表繼續留在 `entry.html`／`entry.plain` 純文字裡。
  - **使用者裁決③明細頁**：保留獨立明細頁。`location` 欄（19 筆同值「礦山
    隧道（雙村共通）」零鑑別度，U29 曾移到 `detailColumns` 但仍逐條重複）
    整個從 `collectionConfigs.js` 拿掉，改成 `CollectionPage.jsx` 新增
    `config.note` 頁面層級一次性說明（「本頁礦物與寶石皆產自礦山隧道
    （雙村共通），下列不逐筆重複標示地點。」），渲染在 h1 下方、搜尋列
    之上；`CollectionPage.jsx` 新增的 `config.note` 是通用機制，非 minerals
    專屬，其餘 collection 需要同類頁面說明時可直接沿用。
  驗證：新增 2 則 `stripMineralFillerIntro` unit test（純填充句整句剝除／
  帶獨有資訊原樣保留）共 231 全過；`npm run lint`／`npm run build` 皆綠、
  build 警告維持 55（無新增/減少）；JSON 產物核對奧利哈鋼（開頭句消失、
  直接從賣價表起頭）、銀／粉紅鑽石（開頭句完整保留）皆符合預期。**未做的
  驗證**：本次無瀏覽器工具，`/c/minerals` 頁面 note 文字排版、明細頁少了
  地點列後的視覺留白是否協調未經核對，麻煩使用者複核。
...
### 2026-07-23 使用者回饋（料理查詢：總覽模式，待實作）

- [x] U56 [UX] `/c/recipes` 補「總覽模式」查詢能力（2026-07-23 完成；**範圍
  已收斂**：原規劃含「食材找料理」篩選＋「總覽模式」兩塊，出 artifact 視覺稿
  對稿時使用者回饋「通常不知道自己可以做什麼，也不想手 key」——玩家記不住
  遊戲內食材日文/中文名，被迫手動輸入食材是額外門檻，「掃一遍自己認」才是
  實際會用的動線，**食材找料理篩選整個拿掉不做**，只留總覽模式）：使用者
  指出來源網站（食譜 blog 原文）可一次看到 70 幾道料理、自己掃過去認食材
  決定要做什麼，現站做不到——列表在 U28（2026-07-20）精簡成「名稱＋5★賣價」
  一行卡，食材要點進單一條目頁才看得到，無法一頁掃視比對。
  做法：`collectionConfigs.js` 的 recipes 設定加 `overview: true` 開關旗標
  （只有宣告的 collection 才會出現切換鈕，非全站硬編碼判斷）；`CollectionPage.jsx`
  加「總覽模式」切換鈕（局部 `useState`，**不進 URL**——這是純畫面顯示偏好、
  不是查詢條件，跟篩選/搜尋的「URL 即狀態」原則不同，離開頁面或重新整理就
  重置是預期行為）；`EntryCard.jsx` 新增 `RecipeOverviewCard`，`CollectionEntryList.jsx`
  轉傳 `overview` prop 決定渲染哪一種卡片。**對稿時的誤判與修正**：
  artifact 視覺稿誤把「現有列表」畫成手帳點狀虛線列，實際核對真正的
  `SingleColumnCard`（U28 既有版型）是村色底色圓角卡片（`bg-(--village)/10
  rounded-2xl`），總覽卡沿用同一張卡片語言往下延伸（卡片內加廚具小標籤＋
  食材 chips 換行），不是另起一種手帳列表樣式——沒有引入新視覺語言，比對稿
  當時設想的還更貼近既有元件。**實作細節（HTML 有效性考量）**：食材 chip
  本身也是連結（`ItemChips` 點食材可跳去該食材條目頁），若整張卡片外層包
  一個 `<a>`（點名稱去料理條目頁）會變成 `<a>` 巢狀 `<a>`（無效 HTML，
  瀏覽器會自動拆解導致點擊行為錯亂）——改成外層 `<div>` 當卡片容器，名稱／
  價格那行自己包一個 `<a>`，跟下方食材 chips 的 `<a>` 是手足關係。
  驗證：`npm run lint`／`npm test`（229，無新增測試——純顯示層開關，無新增
  邏輯分支）／`npm run build` 皆綠；本機起 `npm run dev`＋Playwright 截圖
  核對手機（390px）單欄與桌機（1280px）三欄排版皆正確、卡片高度隨最長食材
  清單（烤魚 12 選 1）自動撐齊同排、`document.querySelectorAll('a a')` 核對
  巢狀連結數為 0、console 無錯誤。
...
使用者以「攻略網站 UX/IA 專家」角度提出三層回饋，逐一處理：①篩選要不要拿到
全站層級（評估後判斷不適合，篩選欄位跨 collection 幾乎不重疊，會議紀錄見
對話）；②篩選列「展開/收合＋只顯示數字」與市場慣例（Spotify/YouTube 常駐
chip、eBay/Airbnb 已選 chip 摘要）不符，出 artifact 對稿三個方向（現況／
補摘要 chip／拿掉收合常駐展開），**尚未拍板，待使用者選定方向**；③使用者
截圖指出同一小塊畫面疊了三條線（header 虛線／搜尋框虛線／篩選陰影），已併入
同一份 artifact 補充比較並確認範圍含 header（見上方 U65 追加段），**修法已
規劃但使用者要求先只記 todo、不要動代碼**。上述三項皆未動工。

- [x] U66 [UX] 移除 `recipes` 列表卡的「總覽模式」切換鈕，固定顯示食材/廚具
  （2026-07-24 完成）。使用者對 U56 總覽模式的存在提出質疑：U28
  （2026-07-20）把列表卡縮成「名稱＋5★賣價」一行卡的前提是「玩家知道菜名
  只想查價錢」，但這款遊戲的料理查詢動機幾乎都是「手上有這些食材/廚具能做
  什麼」——沒開始做菜就不會記得菜名，一行卡對應的情境本來就不成立；也是
  全站唯一把資訊縮進切換鈕後面的 collection，跟其餘 9 個 collection 列表卡
  都直接顯示欄位的慣例不一致，使用者裁決直接拿掉切換鈕、固定顯示。
  做法：`collectionConfigs.js` recipes 設定拿掉 `overview: true`，改
  `richCard: true`（語意從「可切換的模式旗標」改成「這個 collection 用哪種
  卡片元件」）；`EntryCard.jsx` 的 `RecipeOverviewCard` 更名 `RecipeCard`
  （不再是「總覽」這個可切換的次要模式，就是 recipes 唯一的卡片）；
  `CollectionPage.jsx` 拿掉 `overview` state 與「總覽模式」切換鈕、
  `CollectionEntryList.jsx`／`EntryCard.jsx` 一併拿掉 `overview` prop 穿線
  （純 UI 狀態，無殘留邏輯分支）。
  驗證：`npm test`（229，無新增測試——純刪除切換分支，行為由既有
  `EntryCard`/`CollectionEntryList` 測試覆蓋）／`npm run lint`／
  `npm run build` 皆綠（警告數維持 55 則，未新增）。**未做的驗證**：本次
  無瀏覽器工具，`/c/recipes` 頁面實機視覺（食材/廚具是否正確固定顯示、
  總覽模式按鈕確實消失）未經核對，麻煩使用者複核。

### 2026-07-24 使用者回饋（料理卡片點擊範圍過窄，待實作）

- [x] U67 [UX] `EntryCard.jsx` 的 `RecipeCard`（U66 剛拿掉切換鈕、變成 recipes
  唯一卡片）目前 `<a href="#/c/recipes/...">` 只包住名稱＋5★賣價那一行
  （`EntryCard.jsx:36-42`），廚具（`entry.cookware`）跟食材
  （`entry.ingredients`）兩段 `<p>` 是外層 `<div>` 的手足、在 `<a>` 之外，
  完全點不到。使用者回饋：整張卡片（含底下食材那幾行）都應該可點，不是只
  有標題那一小條——卡片視覺上已經用 `bg-(--village)/10 rounded-2xl` 呈現成
  一個完整色塊，點擊範圍卻只有一小條，體感不一致；且這是共用元件，全站每
  張料理卡都有同樣問題，不是單一個例。（2026-07-24 完成）
  做法：`<a>` 直接扛起原本外層 `<div>` 的版型 class（`bg-(--village)/10
  poke-tilt flex h-full flex-col gap-1.5 rounded-2xl p-3`），名稱/價格那行
  改成內層 `<div className="flex items-center justify-between gap-3">`，
  廚具/食材兩段 `<p>` 移進 `<a>` 內、拿掉外層 `<div>` 容器；食材文字仍是
  純文字（U60 裁決不變，未恢復 `ItemChips` 可點特性），只是現在被包在通往
  料理明細頁的大 `<a>` 裡面。
  驗證：`npm run lint`／`npm test`（229，無新增測試——純結構調整，行為由既有
  `EntryCard` 相關測試覆蓋）／`npm run build` 皆綠（警告數維持 55 則）。
  **未做的驗證**：本次無瀏覽器工具（`node_modules` 未裝 playwright），
...
- [x] C29 chip 顯示文字與目標條目頁標題不一致，全站排查（2026-07-24 使用者
  ...
  **已排查（唯讀分析，寫了一支 scratchpad 腳本比對 `src/data/*.json` 的
  `lovesLinks`／`likesLinks`／`hatesLinks`／`loathesLinks`／`ingredientsLinks`
  五個欄位——build 時由 `scripts/itemIndex.js` 的 `resolveOne` 產生，每個
  項目有 `{ zh, jp, href }`，`zh` 是「引用來源自己寫的中文字串」、`href`
  是解析出的目標條目連結。逐一取 `href` 對應目標條目的 canonical `name`／
  `title`，跟 `zh` 比對是否一致）：

  - **不是單一寫錯，是全站性、系統性的現象**：459 次出現、177 組不重複的
    （chip 顯示字／目標條目名）配對，分布在 characters 的 loves/likes/
    hates/loathes（405 次，跨幾乎所有 32 位角色）與 recipes 的
    ingredients（54 次）。抽樣核對十幾組（如「艾莉草→艾利草」「蕃茄→
    番茄」「燉魚→紅燒魚」「毒菇→毒蘑菇」）確認皆是同一物品/料理的中文
    譯名差異，不是解析錯誤指向錯的目標（href 命中的都是對的條目，只是
    顯示字跟目標條目自己的中文名不同）。
  - **根因已知、非新發現**：[content-pipeline.md 規則 6](modules/content-pipeline.md)
    早已明文「跨來源物品參照以 name_jp（日文）為主鍵、中文名為輔鍵——
    中文譯名在不同來源不穩定」。`resolveOne`（`scripts/itemIndex.js`）
    的設計就是保留「引用來源自己怎麼寫」的中文字串來顯示、只用日文鍵找
    連結目標，這是原本的設計選擇，不是實作疏漏——但實際效果就是使用者
    看到的「chip 文案跟點進去的頁面標題對不上」，體驗上確實會讓人以為
    連錯了。

  **兩個修正方向，範圍與取捨不同，需要使用者決定**（不是內容單一欄位
  打錯字這種可以直接判斷對錯的情況，是設計選擇）：
  1. **code 側改一行邏輯**：`resolveOne` 在成功解析出 `href` 時，`zh`
     一律改用目標條目自己的 canonical 中文名（`byHref`/`byJp` 已有這個
     值），不再保留引用來源自己的措辭。一次性解掉全部 459 處，不用碰
     `content/` 任何一個檔案。代價：角色講話的措辭味道（「多利亞焗飯」
     這種角色專屬用語）會被統一置換成資料庫正式菜名，讀起來稍微少一點
     「這是這個角色自己在講話」的感覺。
  2. **內容側逐篇校正**：177 組配對，透過 vault skill 流程逐篇進
     `content/` 對應來源檔案，把中文譯名統一改成目標條目的 canonical
     名稱（或反過來——視個案而定，也可能發現目標條目自己的中文名才是
     比較不通順的那個，需要人工逐組判斷，不是機械式取代）。保留「每篇
     引用各自遣詞」的彈性，但工作量遠大於方案 1（177 組，散落在 32+
...
### 2026-07-24 使用者回饋（手機版篩選鈕比搜尋框高，跑版感，待確認修法方向）

- [ ] U72 [UX] collection 頁（截圖為 `/c/crops`）手機寬度下，搜尋框旁的
  「篩選 ▲」鈕明顯比搜尋框高一截，兩者底部沒對齊，視覺上像跑版、質感
  變差。**本項僅記錄回饋，尚未落地實作**（依 CLAUDE.md「先建 todo、後
  改程式碼」規則，等使用者明確說執行才動工）。

  **已定位成因**：`CollectionPage.jsx:82` 的篩選鈕有 `min-h-11`
  （44px，`md:min-h-0` 只在桌機拿掉）——這是刻意的觸控目標大小規則
  （DESIGN.md §響應式行為「觸控目標 ≥ 44px」，T8.3 當時也把追蹤器按鈕
  加大到同樣的 44px），不是意外殘留的樣式。但同一列的搜尋框
  `<label>`（`CollectionPage.jsx:69`）只有 `py-1.5` padding、沒有對應
  的 `min-h-11`，兩個緊鄰的元素套用了不一致的高度規則，才會在手機寬度
  （`md:` 以下）出現高度落差；桌機因為 `md:min-h-0` 拿掉限制，兩者高度
  自然接近，看不出這個問題。

  認領前需要決定修法方向（暫定草案，非拍板）：
  - 搜尋框 `<label>` 也加 `min-h-11 md:min-h-0`，兩者手機寬度下高度一致
    （最直接，但搜尋框視覺上會跟著變高，不確定是否也是使用者想要的）。
  - 篩選鈕改用「視覺尺寸不變、觸控熱區用 `::before` 或多一層透明
    padding 撐大」的常見無障礙手法，維持跟搜尋框一樣的視覺高度，但點擊
    熱區仍有 44px（技術上更貼近觸控目標規則的本意——規則要求的是可點
    擊範圍夠大，不是可視外框一定要跟著變大）。
  - 也需要確認站內其他頁面有沒有類似「觸控目標鈕跟旁邊非按鈕元素並排」
    的組合，若有同樣風險要不要一併檢查（目前只在 collection 頁篩選列
    發現）。
