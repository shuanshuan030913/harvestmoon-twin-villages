...
- [ ] U64 [UX] `CollectionPage.jsx` 的搜尋框＋「篩選」切換鈕列應該 `sticky`
  ...
  不是新技法。**本項僅記錄問題，尚未實作**——認領前需要確認：①只有搜尋框
  那一列要 sticky，還是連展開的篩選面板（`FilterBar`）／總覽模式切換鈕列
  都要一起黏住（面板展開時可能佔用不少高度，全部黏住會不會讓可視內容區域
  縮水太多）；②sticky 區塊需要不透明背景（`bg-cream`，卡片底色），捲動時
  才不會被下方內容穿透看到；③這個 sticky 行為要套到所有 9 個分類頁（沿用
  `CollectionPage.jsx` 共用元件本來就會全部套用），還是只有像 recipes 這種
  長列表的分類頁才需要。
...
- [ ] U61 [UX] `AnimalTracker.jsx` 的 `AnimalRow` 點心累計列「難以閱讀」，
  ...
  **使用者已選定方向（2026-07-23 追加細化）**：選 C，但具體版型不是三方案裡
  畫的「單欄表格式對齊」，而是參照全站既有 `crops`／多欄 `EntryCard` 列表
  已經在用的 RWD 網格（桌機兩張並排、手機版寬度不夠就自動收成一張一張直向
  排列，同一套 `grid-cols-1 md:grid-cols-2` 手法，不是新版型）——即 4 種
  點心兩兩一組排成一橫排，手機窄螢幕自動塌成單欄直向堆疊。
