...
- [ ] U63 [Interface] guide 頁同時出現內文「## 來源」清單與頁尾「原始出處」
  ...
  **本項僅記錄問題與成因，不預設修法**——可能方向包含：①guides 也納入
  `SOURCES_SECTION_COLLECTIONS`，讓內文「## 來源」段跟條目 collection 一樣
  被剝除、結構化成 `entry.sources`，`GuidePage.jsx` 頁尾比照 `EntryPage.jsx`
  同時支援 `sources` 陣列與 `source` 單值，全站頁尾出處列統一同一套渲染／
  措辭；②只統一措辭不動內容結構（如都改叫「出處」或都改叫「來源」），
  內文清單維持不剝除；③其他做法。因為這會動到 50 篇 guide 內容的渲染方式，
  且觸及「guides 保留原文沿革記錄」這條先前明文裁決過的原則（需要確認
  「## 來源」段落是否算在這條原則保護範圍內，還是純粹的出處資訊不算「沿革
  記錄」），**認領前必須先跟使用者確認方向**才能動手。
...
- [ ] U62 [UX] `GameDialog.jsx`（全站對話框關閉鈕）與 `AnimalTracker.jsx`
  ...
  **範圍需要跟使用者確認**：`GameDialog.jsx` 的關閉鈕（每個對話框右上角，
  ...
  框未明確表態。**本項僅記錄問題，不預設答案**——認領前要先問使用者：
  ①只拿掉 `GameDialog` 關閉鈕的框，還是兩處都拿；②拿掉框後裸 × 的可點擊
  範圍/觸控目標怎麼維持（`h-8 w-8` 這個尺寸目前同時也是熱區，改成純文字
  × 若不刻意保留同尺寸的透明可點擊區，行動版點擊熱區會縮水）。
