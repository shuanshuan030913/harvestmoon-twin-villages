...
- [ ] U18 [UX] 攻略總覽頁（guide 瀏覽入口）（2026-07-19 使用者抽驗 C12 時發現：guides.json 53 篇全無瀏覽/索引入口——`guide/:system/:slug` 路由只能被動連過去才進得去，首頁搜尋只索引 9 個 collection 不含 guides，`CollectionPage`/`FilterBar` 機制也沒接 guides；這次戀愛事件兩篇只是先浮現的症狀，不是特例）：設計並實作一個 guide 瀏覽頁，53 篇依 `system`（farming/livestock/mining/cooking/fishing/bugs/romance/life/basics）分組列出。待決事項（動工前先跟使用者對齊，不要一次問答後就自駕）：入口放哪裡（首頁新增分類？獨立路由＋nav 項？）、要不要併入首頁搜尋索引、視覺上比照哪個既有元件（貼紙卡/緞帶列/列表）、DESIGN.md 沒訂的版面規則要先補後做。(dep: 無，但屬 UX 方向須先討論)
