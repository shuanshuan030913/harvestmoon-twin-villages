...
- [ ] U26 [UX] 動物列表/追蹤器的寵物、畜牧分流（**blocked by C21**（category 欄位化）；列表形態屬 UX 方向，動工前與使用者確認）。消費端現況（2026-07-19 查明）：①列表頁——寵物與畜牧混排，species 篩選混層（牛/羊/雞/羊駝/寵物），寵物列的產物欄顯示整句散文「無（純陪伴寵物，無副產品）」；②畜牧追蹤器（AnimalTracker.jsx:130）——新增動物的搜尋涵蓋全部 12 篇，寵物可被加進畜牧追蹤，但點心升級/副產品機制對寵物無意義（寵物照顧是騎乘/抱/訓練，無 treat_requirements，spec 規則 5 的「缺門檻只顯累計」是為資料未補的家畜設計的，不是給寵物用的）。C21 落地後：③追蹤器新增搜尋排除 `category: 寵物`（畜牧追蹤顧名思義只收家畜；寵物照顧提醒是另一個功能提案，不在本項擅自擴）；④列表呈現二擇一裁決——同一列表加 category 篩選＋分區塊呈現（寵物區/畜牧區，路由與 collection 不動），或拆成兩個 collection（URL/導覽都變，成本高）；傾向前者（最小變動，與「維持最小欄位」原則一致）；⑤species 篩選選項隨 C21 真物種化自動修正（動態產生，不用改碼）。（驗證: 追蹤器搜不到寵物、列表分流呈現與篩選正常、寵物條目頁產物欄不再顯示散文佔位、lint/test/build 綠）
