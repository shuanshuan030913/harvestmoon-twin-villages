...
- [ ] U62 [UX] `GameDialog.jsx`（全站對話框關閉鈕）與 `AnimalTracker.jsx`
  ...
  **範圍需要跟使用者確認**：`GameDialog.jsx` 的關閉鈕（每個對話框右上角，
  ...
  框未明確表態。**使用者已確認①：只要關閉按鈕**——`GameDialog.jsx` 拿掉
  圓形描邊改裸 ×；`DeleteAnimalDialog` 的刪除觸發鈕維持現狀不動，範圍確定。
  **本項記錄範圍已確認，尚未實作**——認領時仍要處理②：拿掉框後裸 × 的
  可點擊範圍/觸控目標怎麼維持（`h-8 w-8` 這個尺寸目前同時也是熱區，改成
  純文字 × 若不刻意保留同尺寸的透明可點擊區，行動版點擊熱區會縮水，做法
  可以是文字 × 外面包一層同尺寸但無背景/無邊框的可點擊區域，視覺上看不到
  框但熱區不變）。
