...
- [ ] U19 [UX] 條目頁內文樣板重複清理——strip 機制擴及其餘集合（U17/recipes 同型：資訊列已由 frontmatter 渲染，內文開頭句與樣板 bullet 全量重複）。**共同執行紀律**：每集合動工前先逐篇驗證開頭句/樣板 pattern 齊一（比照 RECIPE_INTRO 全 273 篇驗證），內文獨有資訊（欄位沒有的值、品質/星度註記語意）不可剝——有欄位缺口的先走 C 側補欄位再剝（C11 教訓）。各集合一子項（15–30 分）：
  ...
  - [ ] U19d crops 45 篇：bullet 樣板（購買價/成長天數/賣價/可否重複收成）重複，但「澆水次數」「種子購買地點」只在內文、buy_price 有欄位但條目頁未顯示——**blocked by C14**（欄位補齊）；補齊後 crops 加 detailColumns（購入價/澆水次數/購買地點/**可重複收成**——regrowable 現況只當篩選器、條目頁從未顯示，2026-07-19 U23 分析補列）再剝 bullet。**剝除前置（2026-07-19 U23 分析發現）**：45 篇「賣價」bullet 全帶「（5 星／最高品質賣價，非基礎 1 星賣價）」語意註記、只存在內文，剝 bullet 前必須先把該語意結構化落地（呈現方式走 U23 裁決），否則列表/明細的 700G 會被讀成基礎賣價——資訊靜默消失
  ...
  - [ ] U19g animals 12 篇：說明段購買價與 buy_price 重複，但「購買地點（哪家店）」「購買前置條件（需飼養牛經驗等）」只在內文——欄位化屬 C 側（見 C17），或裁決保留內文不剝（animals 內文獨有內容佔比高，剝的收益最小，可最後做或不做）
...
- [ ] U23 [UX] 作物「列表 vs 明細」差異化裁決（2026-07-19 使用者觀察「列表和明細內容大部分相似」，分析結論記錄＋兩個裁決點，**UX 方向，須與使用者討論**）。**初衷回顧**：列表＝篩選比較的掃描面（季節/村莊/賣價/成長天數 4 個決策欄＋篩選器，spec 定位「白天查詢工具」）；明細＝完整規格卡（全欄位）＋條目獨有內文＋物品連結鏈節點（食譜食材連進來、相關 guide 連出去）＋來源出處。兩者相似是規則 5「條目頁資訊卡沿用 CollectionConfig 欄位」的刻意設計，**但 crops 現況把相似放大成幾乎重複**：明細資訊列只渲染列表同款 4 欄（buy_price 有欄不顯示、regrowable 只當篩選器、water_times/seed_shop 還沒欄位＝C14），內文又用樣板 bullet 把同 4 個值再講一遍（U19d）——明細目前實際增量只剩 5★ 賣價註記、相關連結、來源。**主修正路徑已排定不另立**：C14（補欄）→ U19d（補 detailColumns＋剝重複 bullet），完成後明細自然成為列表的嚴格超集（多出購入價/澆水次數/購買地點/可重複收成），相似觀感即消解。**裁決（2026-07-19 使用者定案）**：① 欄 label 改「賣價（5★）」（`collectionConfigs.js` crops sell_price label，列表/明細同一份定義自動兩邊生效）——**不依賴 C14，可立即實作**，這也是 U19d 剝 bullet 的前置；② 列表維持最小欄位，不加衍生比較欄（結案，不實作）。本項剩餘工作＝實作①後即可關閉。
- [ ] U24 [UX] 條目頁內文標題重複剝除（2026-07-19 使用者發現 crops 明細標題重複，掃描確認同型範圍：crops 45＋minerals 19＋villages 2 共 66 篇內文帶開頭 `# 標題`，與 EntryPage 自己渲染的 `<h1>` 重複；其餘 collection 內文本無 h1，characters/guides 已各自處理過）：build 端剝除內文開頭 heading——與 guides 的 `extractAndStripLeadingHeading` 不同，entry 頁**不需要** displayTitle 回填（內文 h1 如「蕪菁（かぶ）」的日文名已由 name_jp 欄位承接、EntryPage 標題列會顯示），但動工前先逐 collection 抽驗 h1 文字是否存在 title/name_jp 以外的額外資訊（有就不是純剝除，先回報再定案），驗證後直接剝。（驗證: 66 篇 JSON 產物無 `<h1>`、標題資訊無遺失、警告數不增、lint/test/build 綠）
- [ ] U25 [UX] 內文「## 來源」段整併頁尾出處列——對齊 characters 的 UI（2026-07-19 使用者裁決「來源跟出處應該整併成同樣 UI」，掃描範圍：fishes 64＋insects 85＋items 158＋crops 45＋minerals 19＋festivals 19＋animals 12 共 402 篇內文帶 `## 來源` 段，characters 已整併（extractSources＋頁尾弱化出處列）、recipes 已走 frontmatter source＋retrieved、villages 無此段）：build 端把 characters 的做法通用化——各 collection 一律 `extractSources(displayContent)` 產 `entry.sources`＋剝除「## 來源」段（EntryPage 頁尾 sources 分支既有，前端不用改）。**安全閥**：剝除條件＝extractSources 抽取成功且 bullet 數與段內連結數對帳一致，不符（非標準 bullet 格式、段內夾雜其他文字）者保留原段＋build 警告，不做靜默丟失；guides 維持現狀（來源段屬攻略文章沿革，U4 裁決保留）。（驗證: 402 篇 JSON 產物 sources 陣列完整、html 無 `<h2>來源</h2>`、對帳不符清單為零或逐篇人工核可、lint/test/build 綠）
...
- [ ] U27 [UX] 條目頁「送禮名單」——角色喜好反向索引（2026-07-19 使用者裁決定案：recipes 明細與列表高度相似的優化主方案；四種等級**都加**——最愛/喜歡/討厭/最討厭，討厭類是「別送誰」的防呆，與正面名單同等關鍵）。build 端：把 characters 既有的 lovesLinks/likesLinks/loathesLinks 解析結果依 href 目標反向歸戶，產出各條目的送禮名單欄（如 `giftFans: { loves: [{name, href}], likes, hates, loathes }`）；「討厭」目前管線未解析（角色頁只渲染純文字刪除線 chips），需先補 hatesLinks 解析（同 resolveItemStrings 機制，角色頁自身顯示要不要順勢改用連結 chips 由實作時定，最小範圍是只供反向索引用）。前端：EntryPage 加四區塊（沿用角色頁 chips 視覺語言：最愛 love variant／最討厭 loathe variant），無資料整區不渲染。**機制做通用**（依 href 歸戶天然涵蓋 crops/items/fishes 等被喜好指到的條目，「誰喜歡蕃茄」同樣有送禮價值），可先在 recipes 驗證再全面開啟。已知邊界：類別字串喜好（紅茶系/酒類全般等）未解析成連結者不會出現在名單（U21 類別機制做好自動變完整，不猜、不硬配）。（驗證: 抽驗炊飯/湯豆腐/通心粉等正反向名單與角色頁喜好互為鏡像、警告數不增、lint/test/build 綠）
- [ ] U28 [UX] recipes 列表縮減——名稱＋5★賣價一行卡（2026-07-19 使用者裁決定案，並確立全站欄位取捨標準已回寫 query-system.md 規則 3：比較值進欄、條件值進篩選）。`collectionConfigs.js` recipes：columns 縮為 sell_price_5star 一欄；「分類」欄移除（篩選器既有、全權承擔）、「廚具」欄轉篩選器（uniqueOptions 動態產生）。**明細不丟資訊**：分類（含 guideHref 連結）與廚具移入 recipes `detailColumns`——EntryPage 與列表共用 config.columns，直接刪欄會連明細一起消失，必須同步補 detailColumns。EntryCard 單欄時的版型（名稱左、值右一行）與名稱是否帶日文（全站「中文（日文）」慣例 vs 卡片簡潔）對齊 DESIGN.md，沒定的先補進 DESIGN.md 再實作。（驗證: 列表單行卡呈現、分類+廚具篩選正常、明細頁分類/廚具/賣價三列俱在且分類仍連 guide、lint/test/build 綠）
...
- [ ] C14 crops 欄位補齊（2026-07-19 全站同型問題掃描發現，C11「先補欄位再剝內文」同模式）：45 篇作物條目的「澆水次數」只存在於內文 bullet（如蕪菁「澆水次數：5 次」）、「種子購買地點」只存在於開頭句（如「此花村古恩貝種子屋」），frontmatter 皆無對應欄位；走 vault skill 逐篇核對來源後補 `water_times`／`seed_shop`（或依來源實際欄名定名，來源沒有的照實不寫）。補齊後解除 U19d blocked（code 端加 detailColumns 並剝內文樣板 bullet）。
- [ ] C15 festivals `name_jp: 原文未提供` 佔位值 10 篇（2026-07-19 掃描發現，違反 C9「來源沒有就是沒有」原則——C9 當時清了說明段與 occupation 佔位，漏了這批 frontmatter 值）：作物祭/動物祭/料理大會/南瓜祭/天體觀賞日/星夜祭/雪節/音樂節/春之感謝祭/冬之感謝祭 的 `name_jp` 填了「原文未提供」字面值，且 `EntryPage.jsx:85` 會直接渲染成標題「作物祭（原文未提供）」——**使用者可見的顯示瑕疵，優先處理**。走 vault 流程：先 curl 來源頁（5012066366）核對日文名是否真的沒有（節日名如「収穫祭」很可能原文有、當初 ingest 漏抓），有就補真值，沒有就移除欄位留空。
