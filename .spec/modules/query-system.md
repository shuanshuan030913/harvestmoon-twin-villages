...
8. **列表／明細分拆划算性審查（2026-07-23 使用者裁決，U58）**：新增或調整
   collection 的 `columns`／`detailColumns` 設計時，須檢查明細頁「獨有」內容
   （不在列表欄位、也不跟列表欄位重複的部分）是否單薄且不具跨條目鑑別度。
   若是，優先考慮把該內容結構化後併入列表本身（比照料理總覽模式做法：把有
   意義的獨有資料直接攤在列表卡），而不是預設維持「列表精簡＋明細補完整」
   兩層分拆。背景：同一病灶（明細頁內容稀薄又跟列表高度重複，逼玩家點進去
   卻拿不到新資訊）在 minerals（2026-07-21）與 recipes（2026-07-23）各被
   使用者截圖抱怨後才臨時抓到一次，本規則的目的是把「划算性檢查」變成設計
   新／調整既有 collection 欄位時的例行步驟，不再靠使用者事後發現。
   2026-07-23 全站 9 個 collection 現況核對（口頭核對，非逐值腳本比對）：
   characters／crops 明細獨有內容量大，不受影響；fishes／insects／items
   列表欄位本身已夠豐富，明細稀薄但無「藏資料」問題；festivals／villages
   未細查；animals／pets 未列入本輪核對；已確認需要處理的只有 minerals
   （見下方「後期規劃」與待辦 U59）。
