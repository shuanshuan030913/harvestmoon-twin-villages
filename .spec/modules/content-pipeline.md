...
## Guide 內容審查原則（2026-07-22，U19/U20/C24 執行後歸納）

`guides` collection（長文攻略）與條目 collection（crops/recipes/fishes/…）並存，前者職責是條目/列表頁**表達不出來**的知識，不是條目資料的複讀。審查一段 guide 內文是否該留，用這條判準：

> **拿掉這段，讀者會不會失去「點進條目頁或列表頁篩選也拿不到」的資訊？** 拿不到 → 保留；點兩下就有（甚至更準、更即時）→ 刪或併掉。

**保留類**（條目/列表頁結構性做不到）：
1. **系統機制說明**——廚具對應邏輯、地區代碼怎麼讀、敲礦技巧、隧道任務解法、特定子分類的取得管道（如某些食材/道具只能靠特定機制取得，此類說明即使掛在子分類標題下也算數，不是複讀）。
2. **時序／解鎖策略**——「第 2 年才開始隨機出售」「需先養過牛」這類跨條目關係，單一條目本身講不出來。
3. **編輯策展，但有效期限**——如「5★ 賣價排行 Top 10」，成立前提是「全站目前沒有依價格排序的查詢功能」；此前提一旦改變（如列表頁加了排序 UI），該段即比照下方「刪或併掉」處理，需重新審查。
4. **解讀輔助對照表**——材料分類 glossary、地區代碼表：這些是條目資料的「使用說明書」，本身不重複任何條目的資料值。

**刪或併掉類**（已知反例，供未來對照）：
- U20 前：guide 內逐項食材/賣價全表，與條目 100% 重複。
- C24：談本站自己建置流程的段落（「條目化狀態」「資料矛盾說明」等引用 collection 路徑、frontmatter 欄位名、內部任務代號）。
- 2026-07-22：子分類拆成多個 `##` 標題，但內容只剩「N 道／N 種已收錄於... recipes 條目」——複讀 collection 自己的筆數，且是寫死值、會跟實際資料脫鉤（見 todo C25）。**注意**：拆分子分類本身不是問題，問題是子分類標題下**只有**筆數複讀；若子分類下還帶著保留類 1（機制/取得管道說明），該段落仍要留，只是把複讀句拿掉。
