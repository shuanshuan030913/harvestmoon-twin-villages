...
（依 nth-child 輪替 -1.4° / 0.9° / -0.5°）。

**容器改 flex-wrap，不用 grid**（2026-07-22 修正）：首頁貼紙牆原本是 `grid`，
最後一列不滿版時會貼齊左邊留白；改成 `flex flex-wrap justify-center`＋每張貼紙
固定寬度（`basis-[calc(33.333%-0.5rem)] md:basis-[calc(20%-0.8rem)]`，扣掉 gap
換算成 3 欄／5 欄等寬），讓不滿版的最後一列自動置中。寵物／物品／攻略原本是
獨立緞帶列，同次修正併入這個貼紙牆容器（跟其他 collection 入口同款式），
不再需要另外的緞帶列版面。
...
### 緞帶列（ribbon）
全寬入口列：`bg-soil/15`、虛線邊框、圓角、左圖示右箭頭。**2026-07-22 起縮編為單一
用途**——只剩行事曆的行動版入口（桌機併入貼紙牆，見下方貼紙卡容器改法）。原本
「新入口沒有對應貼紙格時疊加一條緞帶列」的做法（U18，2026-07-19；曾經是攻略總覽、
寵物、物品共 3 條堆疊）已改採貼紙牆改 flex-wrap＋置中不滿版最後一列的方式收納，
不再需要緞帶列堆疊；堆疊多條時容易讓人誤讀成階層關係的問題（寵物曾被誤讀成攻略的
子項）也隨之消失。若之後又有新入口沒有明顯理由能併入貼紙牆（例如需要更醒目的
CTA 呈現、不適合跟其他 collection 同視覺權重），才重新考慮加緞帶列，屆時排序仍要
注意不要暗示錯誤的從屬關係。
...
- ❌ 純白底、冷灰、藍紫漸層。
- ❌ 柔陰影（拍立得除外）、發光效果。
- ❌ 在 seal 紅之外再引入新的強調色。**唯一例外（U71，2026-07-24，使用者
  明確裁決）**：最愛 chip 愛心圖示 hover 實心填色用 `--color-heart`
  桃紅（`#e85d8a`），不是 seal 紅——「最愛」語意上更貼近桃紅而非印泥紅，
  使用者要求跳脫本條規則。僅此一處，不擴大套用到其他強調色場景。
